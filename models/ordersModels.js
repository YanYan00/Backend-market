...
const obtenerPedidosDB = async(id)=>{
    try {
        const consulta = 'SELECT d.idetalle,d.idPedido,d.idProducto,d.cantidad,d.estado,d.precio, p.nombre,p.urlImagen,pe.nombreComprador,pe.direccionComprador FROM detallesPedido d JOIN Pedidos pe ON d.idPedido = pe.idPedido LEFT JOIN Productos p ON d.idProducto = p.idProducto WHERE d.idVendedor=$1';
        const values = [id];
